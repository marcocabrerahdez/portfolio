...
const GlowCard = ({ children, identifier }: Readonly<GlowCardProps>) => {
  useEffect(() => {
    const CONTAINER: HTMLElement | null = document.querySelector(`.glow-container-${identifier}`);
    const CARDS: NodeListOf<HTMLElement> = document.querySelectorAll(`.glow-card-${identifier}`);
    ...
    const RESTYLE = () => {
      ...
      CONTAINER.style.setProperty('--direction', CONFIG.vertical.toString() ? 'column' : 'row');
    };
